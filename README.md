...
Последняя веб-версия в этой ветке: **2026.09.09.1 от 9 сентября 2026 года**. Исполнитель: **Codex / GPT-6**. Версия npm: `2026.9.9-1`. Подробная исходная сборка сохранена как **2026.09.08.4**.

На публичном адресе [prgz.ru/komplektacii4](https://prgz.ru/komplektacii4/) проверена **2026.09.09.1**, исходный коммит `4b98b34`. Все 24 публичных файла совпали по SHA-256; 11 браузерных сценариев прошли. [Протокол публикации](docs/s3000-web-publication-20260909.json) и [измерения до/после](docs/s3000-performance-20260909.md) сохранены в репозитории. Прежняя версия доступна в [архиве 2026.09.08.4](https://prgz.ru/komplektacii4-v2026.09.08.4/), все 17 файлов резервной копии проверены. Методика и решения описаны в [плане оптимизации](docs/s3000-performance-design-20260909.md).

Веб-сетка содержит **1 163 573 треугольника** вместо 3 164 288. Все 49 выбираемых компонентов, 27 карточек, обе схемы питания, горелка и деаэратор сохранены. Четыре отдельные GLB дают повторное использование модели из кеша. Во время вращения не пересчитываются неподвижные тени и не выполняется поиск детали под движущимся указателем. После остановки отрисовка прекращается; переходы камеры и обычные клики продолжают работать.
