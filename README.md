...
Последняя версия в этой ветке: **2026.09.08.2 от 8 сентября 2026 года**. Исполнитель: **Codex / GPT-6 Astra**. Версия npm: `2026.9.8-2`.

Конфигуратор открывает S-3000 с навесным оборудованием из выбранных колонок АДЛ листа S-3000. Экономайзер PREMIUM EQS2 повёрнут на 90° и совмещён с задним дымовым патрубком исходного котла. Оборудование можно выбирать в 3D или искать в списке. Карточка показывает название, количество, происхождение геометрии и сведения из спецификации.

![Сборка со стороны приборов](docs/previews/s3000-front-right.png)

В сборке 27 позиций спецификации, представленных 37 единицами или комплектами. Вместе с корпусом, дополнительными модулями и соединительными элементами — 46 логических компонентов. Горелка, экономайзер и деаэратор включаются отдельно. Выбор дополнительных модулей сохраняется в адресе страницы.

| Открытие | Результат |
| --- | --- |
| `dist/index.html` или корень сервера | S-3000, АДЛ, 8 бар; горелка и экономайзер включены |
| `?addons=` | Дополнительные модули выключены |
...
Новая сборка вместе с декодером и моделью встроена в `dist/index.html`. Для прежнего режима `assembly=legacy` нужны также соседние GLB из `dist`. При размещении всей поставки копируйте **весь каталог dist**. Эта версия подключается через iframe как самостоятельный просмотрщик. Контракт `postMessage` прежнего прототипа в новом режиме не реализован.

Сайт компании не опубликован и не обновлён этой работой. На GitHub переданы исходники и готовая веб-сборка для просмотра и согласования.

## Геометрия и точность

Котёл, экономайзер и деаэратор взяты из исходной CAD-библиотеки. Десять моделей приборов получены со [страницы 3D-моделей ATECH](https://atech-ltd.com/?page_id=9984); ссылки, имена файлов и SHA-256 сохранены в [sources.json](src/assets/s3000/sources.json). Длина стержней LP200/LP400 изменена с 500 до 1000 мм по выбранной спецификации. Для PCF20 и привода нижней продувки использованы доступные близкие исполнения; различия описаны в [технической записке](tools/s3000/README.md).

Указатели уровня RLG-16, арматура АДЛ, насосы Jetex, шкаф и внешний вид горелки построены параметрически с опорой на фотографии. Фотографии относятся к S-4000; количество и наименования оборудования относятся к S-3000. Трассы обвязки и размеры деталей, восстановленных по фото, требуют проверки по монтажным чертежам. Выбор модели горелки для мощности S-3000 не подтверждён расчётом. Эта поставка предназначена для визуального согласования. Инженерная приёмка и подготовка STL для печати не выполнены.
...
- [Результаты локальных проверок](docs/s3000-verification-20260908.json).
...
Исходные фотографии, Excel-файлы с ценами, локальные настройки и архивы загрузок не включены в публичную поставку.
