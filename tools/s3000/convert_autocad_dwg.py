    ...
    for name, insert in inserts.items():
        ...
        for mesh in doc.blocks[name]:
            ...
            meshes.append(dict(vertices=len(vertices), faces=len(mesh.faces), triangles=triangle_count,
                               ...
                               topology_sha256=face_hash.hexdigest(), rgb=list(mesh.rgb),
...
def convert(source, executable, private):
    private.mkdir(parents=True, exist_ok=True)
    output = source.with_suffix('.dwg')
    assert not output.exists(), 'Preserve previous DWG before re-export'
    ...
    for name, block in expected['blocks'].items():
        ...
        for index, (a, b) in enumerate(zip(block['meshes'], received['meshes'])):
            for field in ('vertices', 'faces', 'triangles', 'topology_sha256', 'xyz_sha256_1micron', 'rgb'):
    ...
    report = dict(status='PASSED_AUTOCAD_SAVE_REOPEN_ROUNDTRIP', file=output.name,
                  ...
                  topology_preserved=True, coordinates_preserved_at_mm=.001, colours_preserved=True,
                  audit_errors_on_save=0, audit_errors_on_reopen=0,
    ...
    script.rename(private / script.name)
...
if __name__ == '__main__':
    ...
    a = p.parse_args()
    convert(a.source.resolve(), a.autocad.resolve(), a.private.resolve())
