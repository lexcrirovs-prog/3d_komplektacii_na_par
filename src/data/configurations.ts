...
export interface PartDef {
  ...
  position?: Vec3
  scale?: number
  /** Элемент безопасности — подсвечивается в режиме «Безопасность» */
  safety?: boolean
}
 ...
 * Пиктограммы — инлайновые SVG по key, см. PillarIcon.tsx */
...
export const configurations: Record<string, ConfigurationDef> = {
  ...
  comfort_plus: {
    ...
    summary:
      'Котёл сам себя проверяет и держит давление ровно (±0.2 кг/см²). Максимум безопасности: 2 аварийных + 2 рабочих датчика уровня с ежедневной самодиагностикой.',
    pillars: {
      readiness: 'Полностью автоматизированное заводское изделие',
      safety: '2 аварийных + 2 рабочих датчика уровня с ежедневной самодиагностикой',
    ...
    compare: {
      ...
      water: '2+2 датчика уровня с самодиагностикой',
      feedback: 'Самодиагностика каждый день + диспетчеризация',
      steam: 'ГПЗ с электроприводом',
    },
    parts: [
      // УТП безопасности: вместо одного обобщённого датчика уровня —
      // 2 аварийных + 2 рабочих (конкуренты обычно ставят 1 аварийный).
      // Одиночный level_sensor из Стандарта в этой комплектации скрывается,
      // см. resolveConfigParts в useConfigurator.ts
      {
        id: 'level_sensor_emergency_1',
        label: 'Аварийный датчик уровня №1',
        model: 'level_sensor',
        position: { x: 0.75, y: 1.72, z: -0.3 },
        color: '#ef5350',
        safety: true,
        simple: 'Первый аварийный датчик — следит, чтобы вода не ушла ниже критической отметки.',
        benefit:
          'При упуске воды мгновенно глушит горелку — котёл не перегреется и не выйдет из строя. Проверяет сам себя каждый день.',
        description:
          'Аварийный датчик нижнего уровня №1. Независимый канал защиты от упуска воды (сухого хода). Ежедневная автоматическая самодиагностика по циклу ПЛК; резервируется аварийным датчиком №2.',
      },
      {
        id: 'level_sensor_emergency_2',
        label: 'Аварийный датчик уровня №2',
        model: 'level_sensor',
        position: { x: 0.45, y: 1.72, z: -0.3 },
        color: '#ef5350',
        safety: true,
        simple: 'Второй аварийный датчик — дубль первого. Обычно ставят один, мы ставим два.',
        benefit:
          'Резервирование: даже если один датчик откажет, защита от перегрева при упуске воды сохранится.',
        description:
          'Аварийный датчик нижнего уровня №2 — резервный независимый канал защиты. Вместе с датчиком №1 образует резервированную схему с ежедневной самодиагностикой.',
      },
      {
        id: 'level_sensor_work_1',
        label: 'Рабочий датчик уровня №1',
        model: 'level_sensor',
        position: { x: -0.45, y: 1.72, z: -0.3 },
        color: '#fdd835',
        safety: true,
        simple: 'Рабочий датчик — по нему автоматика управляет подпиткой воды.',
        benefit:
          'Уровень держится точно (±20 мм), вода подаётся плавно — давление пара стабильнее.',
        description:
          'Рабочий аналоговый датчик уровня №1 (4–20 мА). Используется ПЛК для модуляции питательной воды, точность поддержания уровня ±20 мм.',
      },
      {
        id: 'level_sensor_work_2',
        label: 'Рабочий датчик уровня №2',
        model: 'level_sensor',
        position: { x: -0.75, y: 1.72, z: -0.3 },
        color: '#fdd835',
        safety: true,
        simple: 'Второй рабочий датчик — резерв и перекрёстная проверка первого.',
        benefit:
          'ПЛК сверяет показания двух датчиков и замечает неисправность раньше, чем она станет проблемой.',
        description:
          'Рабочий аналоговый датчик уровня №2. Резервирование измерительного канала и кросс-контроль показаний в ПЛК.',
      },
      {
