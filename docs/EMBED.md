...
Приложение собирается в обычную статическую папку `dist/` (лёгкий `index.html`
+ хешированные `assets/` и `models/` — модели кэшируются браузером и грузятся
прогрессивно). Поэтому проще всего:

1. Соберите: `npm run build` → получите папку `dist/`.
2. Разместите **папку целиком** на своём домене, например
   `https://kotelpremium.ru/configurator/` (а не на `kotelgavno.ru`).
   Тогда пользователь визуально и по адресу остаётся у вас.

> Fallback: если хостинг позволяет выложить только один файл, есть
> `npm run build:singlefile` — старая самодостаточная сборка
> `dist/index.html` (~14 МБ, все ассеты внутри, без кэширования моделей).
> Для продакшена не рекомендуется.
