...
const root='E:/CodexArtifacts/Boiler-Family-v2026.09.12.2';
...
for(const family of ['small','medium']) {
 const layout=await json(`${root}/${family}/layout.json`);
 ...
 const replace=new Set([...layout.removed,...layout.parts.map(p=>p.id)]);
 ...
 const parts=scene.listChildren().map(n=>{
   ...
   return {id:p.id,label:p.id==='boiler'?`PREMIUM S-${family==='small'?'1000':'3000'}`:p.label,
 ...
 const cabinet=opening.groups.find(g=>g.id==='cabinet');cabinet.pivot=cabinet.pivot.map((v,i)=>v+layout.moves.control_cabinet[i]);
 if(family==='small')opening.groups=opening.groups.filter(g=>g.id!=='boiler');
 else opening.groups[opening.groups.findIndex(g=>g.id==='boiler')]=(await json('src/assets/s3000/opening.json')).groups.find(g=>g.id==='boiler');
 ...
 const out=`src/assets/families/${family}`;await mkdir(out,{recursive:true});
 await io.write(`${root}/${family}/family-web.glb`,doc);
 await writeFile(`${out}/assembly.json`,JSON.stringify({version:'2026.09.12.2',parts},null,2));
 await writeFile(`${out}/opening.json`,JSON.stringify(opening,null,2));
 assert.equal(sha(await readFile(source)),original);
 await writeFile(`${out}/provenance.json`,JSON.stringify({date:'2026-09-12',executor:'Codex / GPT-6',family,status:'VISUAL_LAYOUT_FOR_TESTING',cadScalePreserved:true,sourceS4000Sha256:original,approach,actuatorSource:'Original actuator recipe, same dimensions, separate replacement cable',sourceBodies:family==='small'?['S-1000 BIM approved by user','DA-3 factory CAD + photo jacket']:['S-3000 v2026.09.09.5'],mountingTranslations:layout.moves},null,2));
